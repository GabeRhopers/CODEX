...
export class BootScene extends Phaser.Scene {
  ...
  preload(): void {
    ...
    // when the parallax picker comes back — see gameplay/ParallaxBackground.ts
    // and level/backgrounds.ts, both still fully intact and unused. Castle's
    // starfield is generated directly at a large, fixed 2048x476 size (no
    // matching sky art in the pack) — see generateTextures.ts. The pack's
    // small 24x24 sky tiles (grass/desert/icy/jungle) used to be baked up to
    // that same size and offered as scenes too, but even baked large they
    // still read as an obvious repeating grid of tiny icons up close — see
