import { BOUNCE_TILE, BRICK_TILE, EMPTY_TILE, WATER_TILE } from "./LevelSchema";
 ...
 * type (brick, bounce, water). Which *frame* of the shared tileset a cell
 * renders as is derived here, never stored:
 ...
 *   - BRICK_TILE / BOUNCE_TILE / WATER_TILE: always their own fixed frame,
 *     regardless of neighbors — they're standalone blocks, not merging
 *     terrain.
...
export const GROUND_FRAME_WATER = 4;
...
export function groundFrameAt(grid: number[][], x: number, y: number): number {
  ...
  if (value === WATER_TILE) return GROUND_FRAME_WATER;
