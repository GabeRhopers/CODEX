 ...
 *
 * Lives in support/ rather than inside one spec so a published game — which
 * boots in its own browser context and cannot use the editor's helpers — is held
 * to the same standard as the editor is.
 */
...
export async function boxes(page: Page, sceneKey: string): Promise<Box[]> {
...
export async function assertLayoutSound(page: Page, sceneKey: string): Promise<void> {
