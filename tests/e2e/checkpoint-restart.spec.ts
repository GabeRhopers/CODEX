...
test("restarting after a checkpoint touched in Sub reopens in Sub, not Main", async ({ page }) => {
  ...
  await expect.poll(() => readSceneField<string>(page, "Play", "outcome"), { timeout: 5000 }).toBe("lost");
  ...
  await expect.poll(() => readSceneField<string>(page, "Play", "outcome"), { timeout: 5000 }).toBe("playing");
